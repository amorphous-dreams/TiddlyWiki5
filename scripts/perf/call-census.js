...
Boots an edition, instruments a curated set of core functions to COUNT their
calls, then parses and renders every wikitext tiddler `renders` times (default 3)
and reports the counts.

Why counts, not a profiler? A CPU sampling profiler attributes time to functions
by sampling the call stack, and it gets that attribution wrong — stale frames,
inlined code, and aliasing make it flag functions that barely ran. A call count
is exact. Pair this census with the perftest timings: if the profiler calls a
function hot but this census shows few calls, the profiler is lying. Counting is
how you catch the ghost before you optimise it.
*/
"use strict";
